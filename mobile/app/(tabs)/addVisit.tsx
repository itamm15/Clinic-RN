...
export default function AddVisitScreen() {
  ...
  }, [patients, doctors, patientsLoading, doctorsLoading]);

  const handleSubmit = async () => {
    if (!visitDate || !visitTime || !visitReason || !selectedPatientId || !selectedDoctorId) {
      Alert.alert('Błąd', 'Uzupełnij wszystkie pola.');
      return;
    }

    try {
      const fullDateTime = new Date(`${visitDate}T${visitTime}:00`);

      const response = await axios.post('http://localhost:5183/api/visit', {
        visitDate: fullDateTime.toISOString(),
        visitReason,
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Dodaj wizytę</Text>

      <TextInput
        style={styles.input}
        placeholder="Data wizyty (YYYY-MM-DD)"
        value={visitDate}
      ...
      <TextInput
        style={styles.input}
        placeholder="Godzina wizyty (HH:mm)"
        value={visitTime}
